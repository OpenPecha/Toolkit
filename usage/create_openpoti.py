...
def main():
    ...
    ap.add_argument("--output", type=str, help="Path to export all the Openpoti")
    args = ap.parse_args()

    formatter = OPFormatter(output_path=args.output)

    for m_text_file in tqdm([o for o in list(Path(args.input).iterdir()) if str(o).endswith('.txt')]):
