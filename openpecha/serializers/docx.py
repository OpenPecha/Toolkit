...
def format_span(span, p, document):
    ...
    if span['class'][0] == "tibetan-root-text-in-verse":
        ...
        p.style = document.styles['Citation_verse_first_line']
        Citation = p.add_run(f'{span.text}')

    elif "tibetan-citations-in-verse-middle-line" == span['class'][0]:
        p.style = document.styles['Citation_verse_middle_line']
        Citation = p.add_run(f'{span.text}')
...
def format_p_tag(p_tag, p, document):
    ...
    if 'tibetan-regular-indented' in p_tag['class'][0]:
            p.style = document.styles['Indented_commentary']
    for content in p_tag.contents:
        if 'span' == content.name:
            format_span(content, p, document)
        else:
            normal_text = p.add_run(f'{content}')
            normal_text.style = document.styles['Commentary']
            if page_break_needed(p_tag):
                normal_text.add_break = WD_BREAK.PAGE

def add_styles(doc_styles):
...
def create_docx(html, output_path, pecha_id):
    ...
    """
    p_tags = get_all_p_tags(html)
    document = Document()
    doc_styles = document.styles
    add_styles(doc_styles)
    for p_tag in p_tags:
        p = document.add_paragraph()
        if p_tag.attrs and p_tag['class'][0] == 'credits-page_epub-edition-line':
            add_credit_page(p_tag, p)
        else:
            
            format_p_tag(p_tag, p, document)
