...
web_meta_path = catalog_data_path/'website_metadata.json'  # save metadata in .opf/meta.yml
ebook_meta_path = catalog_data_path/'ebook_metadata.csv'     # replace ID with new Pecha ID
...
def create_meta_fn(path):
    ...
    for o in web_meta:
        if o['sku'] == sku:
            ...
            yaml.dump(opf_meta, opf_meta_fn.open('w'), default_flow_style=False,  sort_keys=False, allow_unicode=True)
